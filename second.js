document.addEventListener('DOMContentLoaded', () => {
    ...
    let soundUnlocked = false; // ===== SOUND FLAG =====
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.querySelectorAll('.skill').forEach(skill => {
                        const level       = parseInt(skill.dataset.level, 10);
    ...
    setTimeout(() => {
        ...
        if (toastSound && soundUnlocked) {
            toastSound.currentTime = 0;
            toastSound.play().catch(err => console.warn('Autoplay blocked:', err));
        }
    }, 5000);
    ...
    if (toast) {
        toast.addEventListener('click', () => {
            ...
            link.href = 'Raymart-Catudio-CV-2025.pdf';
...
const inspiredText = document.getElementById("inspired-text");
const discordList = document.querySelector(".discord-server-list"); // your left list

toggle.addEventListener("change", () => {
  const isOn = toggle.checked;
  
  // Show/hide top-right text
  inspiredText.classList.toggle("show", isOn);

  // Slide in/out Discord server list
  discordList.classList.toggle("active", isOn);
});
